...
- 只统计服务端确认的已发布、已核验客观题。
- 开启参与时记录授权时间，每次作答固化当时的参与状态；匿名历史和退出期间作答不计分。
- 退出再参与不会重置授权时间，重新参与后恢复原有合规分数。
- 同一道题按参与后的首次正确作答计分，每题最多 1 分。
- 简答题自评暂不计分。
...
1. 已完成：SQLite 分表存储、旧 JSON 迁移和 Schema v1 到 v7 升级。
...
6. 已完成：排行榜参与设置接口与“我的”页开关，默认关闭且只允许登录用户修改。
7. 待完成：AGC 正式配置、生产 HTTPS 和真实华为账号联调。
8. 待完成：账号注销。
9. 已完成：周榜、总榜、分类榜统计接口和 HarmonyOS 排行榜页面。
10. 已完成：排行榜异常频率检测、管理端参与账号分页审计、带原因和内部备注的封禁/解封、可识别操作人、会话吊销和操作记录。
11. 已完成：独立管理员账号、短期会话和角色权限，排行榜操作人由服务端管理员会话确定。
12. 已完成：管理员登录失败持久化限流、统一操作审计、会话查看和强制下线。
