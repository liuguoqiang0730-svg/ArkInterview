...
58. App 一级信息架构已从五项底部导航收敛为“首页 / 训练 / 我的”：首页负责学习总览、训练模式、模块题库和面试专题；训练负责题单与连续答题；我的负责账号和个人学习资产。练习记录、错题本、收藏题目、排行榜已整合到“我的”的学习中心，平板使用两列入口，手机使用单列入口；旧 `navActive` 路由来源状态已移除。HarmonyOS `assembleApp` 构建、打包和签名通过。
...
4. 真机检查新的三级底部导航和“我的”学习中心，重点确认平板两列入口、手机单列入口、页面高亮归属和返回路径。
5. 继续对齐平板真实 UI 和预览方向，重点检查首页右侧滚动、答题页状态浮层、完成总结页和底部导航动效。

优先级 P1：

1. 题库扩充暂缓，下一轮优先优化首页与训练题单的职责边界、返回路径和空状态，避免一级入口出现重复内容。
