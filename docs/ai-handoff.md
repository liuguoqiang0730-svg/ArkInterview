...
- 当前模块题量：ArkTS/ArkUI 各 45 道；调试发布/NEXT 适配各 44 道；relationalStore 42 道；NAPI/性能各 24 道；Stage 模型、组件通信、状态管理各 22 道；其余模块各 21 道。
- 最近一次题库核验：2026-07-29，本批新增 100 道，本地聚合总数 460；线上仍需通过远程发布脚本同步后再核对总数。
...
2. 当前本地题库已有 460 道，数量达到 MVP 基线，但仍需持续补充真实面试场景并定期复核过时 API；本批 100 道尚待发布到线上服务器。
