...
38. 可选华为账号登录后端基础已完成：服务端按 Authorization Code 流程验证身份，签发并轮换 ArkInterview 访问/刷新令牌，首次登录事务合并匿名收藏、错题和答题记录；登录账户使用内部账户锚点，未携带访问令牌时不会通过旧设备 ID 暴露账户数据。HarmonyOS Account Kit 客户端和“我的”页面尚待接入。
...
6. 接入 HarmonyOS Account Kit 客户端与“我的”页面，调用现有 `/api/auth/*` 接口并安全保存 ArkInterview 会话；匿名模式保持完整可用。

优先级 P2：

1. 登录用户云端同步和账号注销。
2. 登录用户自愿参与的周榜、总榜和分类排行榜。
3. 服务端 AI 简答题评分。
4. AI 模拟面试追问。
5. 每日一练。
...
4. 清除应用数据或卸载重装会生成新的匿名设备 ID；后端已经支持通过同一华为账号合并匿名记录，但 App 客户端尚未接入，因此当前版本仍不能从界面完成跨设备恢复。
