...
48. 错题/收藏管理页已完成批量管理和信息密度优化：平板题卡改为双列，手机保持单列；支持跨筛选选择、全选当前筛选、批量标记掌握和批量取消收藏。服务端新增整批校验、SQLite 事务写入的批量接口，过期状态返回 `409` 且不产生部分写入；客户端在旧服务器返回 `404` 时兼容回退单题接口。错题列表现在返回已掌握记录，“未掌握/全部错题”筛选实际可用。
...
4. 错题/收藏管理下一步补充批量操作二次确认、失败状态刷新和真机平板视觉验收。
