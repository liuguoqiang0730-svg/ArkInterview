...
5. 继续对齐平板真实 UI 和预览方向，重点检查首页右侧滚动、答题页状态浮层、完成总结页和底部导航动效。

优先级 P1：

1. 题库扩充暂缓，下一轮继续做答题页与本轮完成总结页的真机 UI 优化，重点检查题干阅读宽度、选项触控区域、解析信息层级、题号状态浮层和总结页错题复盘入口。
...
4. 错题/收藏管理下一步进行真机平板视觉验收，重点检查双列题卡等高、批量工具栏和确认弹窗。
...
1. 训练中心与设置浮层已在 `2560×1600` 横屏平板真机验收；手机短屏目前仍以响应式代码和构建验证为主，后续需要在实际手机或手机模拟器补充滚动、输入法和底部安全区检查。
