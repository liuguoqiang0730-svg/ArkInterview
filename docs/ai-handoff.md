...
8. 平板优先首页布局第一版。
9. 首页底部导航基础可用：错题、收藏进入对应练习，记录进入练习记录页。
...
2. 优化答题页平板布局，让题干、选项、解析更像训练工作台。
3. 梳理底部导航后续形态，避免在二级页面和首页出现交互不一致。
4. 处理 ArkUI deprecated API 警告，优先迁移路由和 Toast。
