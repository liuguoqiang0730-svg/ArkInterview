...
- 当前模块题量：ArkTS/ArkUI 各 45 道；调试发布/NEXT 适配各 44 道；relationalStore 42 道；NAPI 34 道；Stage 模型、组件通信、状态管理各 32 道；Ability 生命周期、路由、网络、权限、数据存储、Preferences 各 31 道；性能优化 24 道。
- 最近一次题库核验：2026-07-29，本批新增 100 道，本地聚合总数 560；线上仍需通过远程发布脚本同步后再核对总数。
...
1. 继续扩充题库，但必须按官方文档确认；下一批优先 Worker/Sendable 深层约束、ArkUI 复用与动效性能、RDB 慢查询/FTS、包体与制品追溯、多屏协同和大字体极限布局。
...
2. 当前本地题库已有 560 道，数量达到 MVP 基线，但仍需持续补充真实面试场景并定期复核过时 API；最新两批共 200 道尚待发布到线上服务器。
