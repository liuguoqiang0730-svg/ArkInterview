...
> 当前产品状态（2026-07-31）：真实华为账号联调和线上启用暂缓。现有客户端与服务端基础实现保留，登录入口继续保持禁用，匿名模式不受影响。恢复该任务前需先确认购买正式域名和可备案服务器，并完成 HTTPS 与 ICP/App 备案。

当前客户端已经配置 OAuth Client ID。当前 DevEco 自动调试证书的叶子证书 SHA-256 指纹为：

```text
DF:21:A3:C0:9F:79:54:57:93:05:F8:5C:64:F8:0C:AD:86:F7:98:53:EE:3A:88:7C:1D:EC:95:D2:18:DF:3A:37
```

该指纹仅对应当前调试签名；DevEco 重新生成签名或正式发布改用发布证书后，需要在 AGC 追加对应证书指纹。
