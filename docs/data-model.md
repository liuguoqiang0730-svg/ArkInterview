...
运行时数据保存在 `backend/storage/arkinterview.sqlite`，当前 Schema 版本为 `7`。题库源文件仍按模块维护在 `data/question-bank/modules/*.json`，构建后同步到 SQLite；用户私有数据不进入 Git。
...
- `admin_sessions`：管理员短期会话令牌摘要、来源 IP、过期时间和吊销状态；数据库不保存明文令牌。
- `admin_login_limits`：按用户名和来源 IP 保存短期失败计数、统计窗口和锁定到期时间；成功登录后清除对应记录，陈旧记录会自动清理。
- `admin_audit_events`：保存管理员身份快照、动作、目标、摘要、受控详情、来源 IP 和服务端时间，形成后台统一审计链路。
...
错题批量标记掌握和收藏批量移除都先校验整批题目 ID，再使用 SQLite 事务同步更新用户记录与明细表，避免客户端逐条请求出现部分成功。错题列表保留已掌握记录供“全部错题”复盘，错题练习池只读取 `mastered = false` 的记录。
...
- 同一用户名和来源 IP 连续登录失败达到阈值后临时锁定，锁定状态持久化到 SQLite，服务重启不会绕过限制。
- 超级管理员可以查看最近 500 条会话并强制下线其他会话或其他账号的全部会话；当前会话必须通过正常退出接口撤销。
